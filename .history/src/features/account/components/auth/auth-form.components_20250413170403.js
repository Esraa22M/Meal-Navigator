	...
	return (
		...
		>
			...
			<Pressable
				...
				style={({ pressed }) => [
					pressed && { backgroundColor: colors.ui.accent, borderRadius: 5 },
				]}
			>
				<Text
					style={{
	...
	const authHandler = useCallback(
		async (email, password, mode) => {
			try {
				if (
					mode === "register" &&
					(isValidInputData?.firstName?.isValid===false ||
						isValidInputData?.lastName?.isValid===false ||
						isValidInputData?.password?.isValid ===false||
						isValidInputData?.repeatedPassword?.isValid===false)
				) {
					setIsLoading(false);
	...
	return (
		...
		>
			<OuterWrapper>
				...
				<LoginContainer
					mode={mode}
					contentContainerStyle={isLogin && { justifyContent: "center" }}
				>
